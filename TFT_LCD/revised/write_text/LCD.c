...
void lcd_write_digit(uint16_t color, uint8_t digit)
{
	if (digit > 9) return;

	uint16_t bg = LCD_TEXT_BG_COLOR;
	...
	for (uint8_t row = 0; row < CHAR_H; row++)
	{
		...
		{
			uint8_t b = (row < 8)
			? pgm_read_byte(&digit_font_12x16[digit][col])
			: pgm_read_byte(&digit_font_12x16[digit][12 + col]);
...
void lcd_write_array(uint16_t color, uint8_t idx)
{
	uint16_t bg = LCD_TEXT_BG_COLOR;
	...
	const uint8_t *glyph = &control_font_12x16[idx][0];    //12x16
	...
	for (uint8_t row = 0; row < CHAR_H; row++) {
		...
		const uint8_t *row_base = glyph + ((row < 8) ? 0 : 12);
...
void lcd_write_char(uint16_t color, char ch)
{
	uint16_t bg = LCD_TEXT_BG_COLOR;
	...
	if (ch >= 'A' && ch <= 'Z') 
	{
		glyph = &alphabet_font_12x16[(uint8_t)(ch - 'A')][0];
	} 
	else if (ch >= '0' && ch <= '9') 
	{
		glyph = &digit_font_12x16[(uint8_t)(ch - '0')][0];
	} 
	else 
	{
		return;
...
void lcd_write_string(uint16_t color, const char *s)
{
	while (*s) {
		lcd_write_char(color, *s++);
	}
}
